...
This document specifies the REST binding for the [Cart Capability](https://sakinaroufid.github.io/ucp-announcement/draft/specification/cart/index.md).
...
- **Idempotency-Key**: Operations that modify state **SHOULD** support idempotency. When provided, the server **MUST**:
  ...
  1. Return `409 Conflict` if the key is reused with a mismatched body. See [Message Signatures — Idempotency Key Requirements](https://sakinaroufid.github.io/ucp-announcement/draft/specification/signatures/#replay-protection) for the full payload-matching contract.
...
See the [Core Specification](https://sakinaroufid.github.io/ucp-announcement/draft/specification/overview/#error-handling) for the complete error code registry and transport binding examples.
...
1. **OAuth 2.0**: Via `Authorization: Bearer {token}` header. Identifies the platform for agent-authenticated access, or both platform and user for user-authenticated access (see [Identity Linking](https://sakinaroufid.github.io/ucp-announcement/draft/specification/identity-linking/index.md)).

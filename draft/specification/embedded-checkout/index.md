...
ECP uses the shared EP transport layer. See [Embedded Protocol — Transport & Messaging](https://sakinaroufid.github.io/ucp-announcement/draft/specification/embedded-protocol/#transport-messaging) for message format, message types, and response handling conventions.
...
ECP follows the shared EP communication channel model. See [Embedded Protocol — Communication Channels](https://sakinaroufid.github.io/ucp-announcement/draft/specification/embedded-protocol/#communication-channels) for the general pattern.
...
`ec.auth` implements the shared EP authentication pattern — see [Embedded Protocol — Authentication](https://sakinaroufid.github.io/ucp-announcement/draft/specification/embedded-protocol/#authentication) for the request/response contract, examples, and error escalation flow.

- **Method:** `ec.auth`
- **Direction:** Embedded Checkout → Host (request); Host → Embedded Checkout (response)

When error escalation is required, Embedded Checkout **MUST** issue an `ec.error` notification per the [session error pattern](https://sakinaroufid.github.io/ucp-announcement/draft/specification/embedded-protocol/#session-error).

### Lifecycle Messages

Lifecycle notifications follow the shared EP pattern — see [Embedded Protocol — Lifecycle](https://sakinaroufid.github.io/ucp-announcement/draft/specification/embedded-protocol/#lifecycle). All lifecycle notifications carry the full `checkout` object as their payload.
...
State change notifications follow the shared EP pattern — see [Embedded Protocol — State Change](https://sakinaroufid.github.io/ucp-announcement/draft/specification/embedded-protocol/#state-change). All state change notifications are sent from the Embedded Checkout to the host and carry the full `checkout` object as their payload.
...
`ec.error` implements the shared EP session error pattern — see [Embedded Protocol — Session Error](https://sakinaroufid.github.io/ucp-announcement/draft/specification/embedded-protocol/#session-error) for the payload specification and host handling requirements.
...
The address object uses the UCP [PostalAddress](/ucp-announcement/draft/specification/checkout/#postal-address) format:
...
See [Embedded Protocol — Error Codes](https://sakinaroufid.github.io/ucp-announcement/draft/specification/embedded-protocol/#error-codes) for the shared error codes. Embedded Checkout defines the following additional codes for delegation-specific scenarios:
...
ECP inherits the shared EP security requirements for CSP, iframe sandboxing, credentialless iframes, and strict origin validation. See [Embedded Protocol — Security](https://sakinaroufid.github.io/ucp-announcement/draft/specification/embedded-protocol/#security) for the full specification.
